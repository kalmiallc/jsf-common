...
export interface JsfLayoutOnClickInterface {
  ...
  openFormDialog?: {
    ...
    /**
     * Whether the event chain should be aborted if the dialog is dismissed (for example when the user clicks on the backdrop instead of
     * submitting the form).
     */
    abortOnDismiss?: {
                       $eval: string;
    ...
    /**
     * Should the form dialog be displayed inside a proxy document (iframe)
     */
    proxy?: boolean;
  ...
  clipboard?: {
    ...
    /**
     * Empty clipboard, clear clipboard cache from following keys
     */
  ...
  showNotification?: {
    ...
    /**
     * Notification style. Defaults to info.
     */
    level?: {
              $eval: 'info' | 'success' | 'warn' | 'error';
